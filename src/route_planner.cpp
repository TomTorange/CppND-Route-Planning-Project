...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
     // *current_node->neighbors.push_back(FindNeighbors());
     current_node->FindNeighbors();
     for (auto neighbour : current_node->neighbors){
       neighbour->parent = current_node;
       neighbour->h_value = CalculateHValue(current_node);
       neighbour->g_value = neighbour->g_value + neighbour->h_value;
...
RouteModel::Node *RoutePlanner::NextNode() {
    ...
    RouteModel::Node *node_min = this->open_list.back();
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    //add distance from current to the parent
    distance += current_node->distance(*current_node->parent);

    //insert the node to the path vector
